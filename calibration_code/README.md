...
- ✅ **相机单目标定**：棋盘格角点自动检测 + OpenCV `calibrateCamera` 求解内参、畸变系数
...
├── chessboard_detector.py    # 棋盘格角点检测
...
<根目录>/
    ...
    │   ├── 3N+1.bmp    ┐
    │   ├── ...         │ 垂直反格雷码 (N 张)
    │   └── 4N.bmp      ┘  ← 最后一张同时作为"全白投影/棋盘格图"
    ├── 2/
    ├── 3/
    └── ...
```

**总图像数 = 4 × bits + 1**（最后一张反格雷码同时用作全白棋盘格图，复用同一文件）。
...
3. **标定参数**：棋盘格列数/行数、方格边长(mm)、格雷码位数、投影仪分辨率、误差阈值
...
python main.py --cli --root <图像根目录> --output <输出目录> \
               --cols 11 --rows 8 --square 15.0 --bits 11 \
               --proj-w 1920 --proj-h 1080 --threshold 1.0
```
...
| 步骤 | MATLAB 工具箱 | 本实现 |
|------|--------------|--------|
| 角点检测 | `detectCheckerboardPoints` | `cv2.findChessboardCorners` + `cornerSubPix` |
| 世界坐标生成 | `generateCheckerboardPoints` | `np.mgrid` 生成规则网格（行优先） |
...
- **角点检测失败**：图像过曝/过暗，或棋盘格贴得不够平整。提高光照对比度，也可尝试调整 OpenCV 标志位。
